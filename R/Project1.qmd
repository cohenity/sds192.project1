Main variable: EDStations, Tot_ED_NmbVsts
Variables being analyzed: PrimaryCareShortageArea, Visits_Per_Station, UrbanRuralDesi, HospitalOwnership, Year, County 
...
Urban_Shortage_Areas <- 
  ...
  na.omit(emergency_department_volume_and_capacity_2021_2023)
```
...
ggplot(Urban_Shortage_Areas, aes(x = EDStations, 
  ...
  labs(x = "Number of ED Stations", 
       ...
       department stations that are relatively compatible with their number of visitors per facility. Nonprofit hospitals
       have the lowest capacity to accomodate large numbers of patients while government owned hospitals have the highest.") +
  theme(plot.caption = element_text(size = 8))
```
...
ggplot(Urban_Shortage_Areas, 
  ...
  labs(x = "County",
       ...
       access to primary care, but also that it is likely the most populous area being evaluated by the data. Hospitals in more 
       populous areas are more likely to have less access to primary care and a lower capacity to efficiently treat patients.") + 
  theme(plot.caption = element_text(size = 8))
```
